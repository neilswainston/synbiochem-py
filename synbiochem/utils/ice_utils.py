...
class ICEClient(object):
    ...
    def __get_sbol_doc(self, ice_id):
        ...
        return SBOLDocument(temp_file.name)
    ...
    def __upload_sbol(self, ice_number, sbol_doc):
        '''Uploads an SBOLDocument to ICE database.'''
        url = self.__url + '/file/sequence'
        files = {'file': str(sbol_doc), 'entryRecordId': str(ice_number)}
...
def main():
    ...
    ice.set_ice_entry(ice_entry)
    print ice.get_ice_entry(ice_entry.get_id())
